...
class Settings:
    ...
    condition_files: List[str] = field(default_factory=list)
    experiment_files: List[str] = field(default_factory=list)
    all_pathways: dict = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        self.output_path = path.join(
            self.root_folder,
            "Outputs",
            "{}_{}.xlsx".format(self.experiment_name, self.date),
        )

        # Adjust the similarity matrix paths based on normalization type
        self.similarity_matrix_path = path.join(
            self.data_dir, "matrix", f"{self.network}_{self.alpha}.npz"
        )
        self.network_file_path = path.join(self.data_dir, "network", self.network)
        self.genes_names_file_path = path.join(
