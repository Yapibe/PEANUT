...
def generate_decoy_pathways(pathway_file, output_file):
    ...
    for pathway, genes in pathways.items():
        decoy_genes = random.sample(
            genes_pool, len(genes)
        )  # Shuffle from the gene pool
...
def evaluate_performance(real_results, decoy_results):
    ...
    y_scores = [
        result["Adjusted_p_value"] for result in real_results.values()
    ] + [
        result["Adjusted_p_value"]
        for result in decoy_results.values()
    ]
...
pathway_file = path.join(
    root_folder, "Data", "Anat", "pathways", "kegg"
)
output_file = path.join(
    root_folder, "Data", "Anat", "pathways", "kegg_decoy"
)
