...
edges_df = pd.read_csv(edges_file, sep='\s+', names=['protein1', 'protein2', 'combined_score'],
                       dtype={'protein1': str, 'protein2': str, 'combined_score': float}, skiprows=1)

# Ensure the combined_score is numeric and divide by 1000
edges_df['combined_score'] = edges_df['combined_score'] / 1000

# Load the second file (node translation dictionary)
dictionary_file = '../Data/Human/network/protein.info'
dictionary_df = pd.read_csv(dictionary_file, sep='\t', usecols=['#string_protein_id', 'preferred_name'])

# Create a dictionary for translation, ensure the keys and values are stripped of any extra whitespace
translation_dict = pd.Series(dictionary_df['preferred_name'].values, index=dictionary_df['#string_protein_id'].str.strip()).to_dict()
...
edges_df['protein1'] = edges_df['protein1'].str.strip().map(translation_dict).fillna(edges_df['protein1'])
...
edges_df['protein1'] = pd.to_numeric(edges_df['protein1'], errors='coerce').fillna(edges_df['protein1'])
edges_df['protein2'] = pd.to_numeric(edges_df['protein2'], errors='coerce').fillna(edges_df['protein2'])

# Identify rows where gene ID has letters
invalid_rows = edges_df[(edges_df['protein1'].apply(lambda x: isinstance(x, str) and any(c.isalpha() for c in x))) |
                        (edges_df['protein2'].apply(lambda x: isinstance(x, str) and any(c.isalpha() for c in x)))]
...
output_file = '../Data/Human/network/String'
