...
edges_df = edges_df.drop(
    columns=["database", "textmining", "combined_score"]
)
...
edges_df["protein1"] = (
    ...
    .fillna(edges_df["protein1"])
)
edges_df["protein2"] = (
    edges_df["protein2"]
    .str.strip()
    .map(translation_dict)
    .fillna(edges_df["protein2"])
)
...
edges_df["protein1"] = (
    edges_df["protein1"].map(gene_info).fillna(edges_df["protein1"])
)
edges_df["protein2"] = (
    edges_df["protein2"].map(gene_info).fillna(edges_df["protein2"])
)

# Convert the gene IDs to integers, and handle cases where the mapping was not successful
edges_df["protein1"] = pd.to_numeric(
    edges_df["protein1"], errors="coerce"
).fillna(edges_df["protein1"])
edges_df["protein2"] = pd.to_numeric(
    edges_df["protein2"], errors="coerce"
).fillna(edges_df["protein2"])
...
print(
    f"Number of rows with gene IDs containing letters: {num_invalid_rows}"
)
