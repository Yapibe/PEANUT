...
species = "Anat"  # Species identifier, used to fetch species-specific files
genes_info_filename = "Anat.gene_info"  # File containing gene information
...
def get_scores(test_name):
    ...
    try:
        ...
        raw_data = pd.read_csv(
            raw_scores_file_path, usecols=["GeneID", "Score", "P-value"]
        )
...
def perform_statist(test_name):
    ...
    for pathway in significant_pathways:
        ...
        pathway_scores = [
            scores["Score"][gene_id] for gene_id in genes if gene_id in scores["Score"]
        ]
...
def perform_statist_mann_whitney(passed_ks_pathway_dict, scores):
    ...
    for i, row in pathways_df.iterrows():
        ...
        if not any(
            jaccard_index(current_genes, set(filtered_row["Genes"])) > JAC_THRESHOLD
...
def read_pathways(file_name):
    ...
    try:
        with open(file_name, "r") as file:
            return {
                line.split()[0]: [int(gene) for gene in line.split()[2:]]
                for line in file
                if line.strip()
            }
    except FileNotFoundError:
...
condition_files_unsorted = [
    os.path.join(temp_output_folder, file) for file in os.listdir(temp_output_folder)
]

# Sort condition_files based on the order of test_names
condition_files = sorted(
    condition_files_unsorted,
    key=lambda x: test_names.index(os.path.splitext(os.path.basename(x))[0]),
)
