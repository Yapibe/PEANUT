...
def run_propagation_and_enrichment(test_name, prior_data, network, network_name, alpha, method, output_path):
    if method in ['PROP', 'ABS_PROP']:
        # Set alpha before initializing GeneralArgs for PROP and ABS_PROP
        general_args = GeneralArgs(network=network_name, pathway_file='kegg_decoy', method=method, alpha=alpha)
        if method == 'ABS_PROP':
            general_args.input_type = 'abs_Score'
    elif method in ['GSEA', 'NGSEA']:
        # Initialize GeneralArgs for GSEA without modifying alpha
        general_args = GeneralArgs(network=network_name, pathway_file='kegg_decoy', method=method)
        if method == 'NGSEA':
    ...
    try:
        ...
        if 'FDR q-val' in results_df.columns:
            ...
            if not results_df.empty:
                percentage_significant = (len(significant_pathways) / len(results_df)) * 100
...
def process_file(network, network_name, alpha, prop_method, file_name):
    ...
    output_dir = os.path.join(output_base_dir, prop_method, network_name, 'Decoy', f"alpha_{alpha}")
    os.makedirs(output_dir, exist_ok=True)
    output_file_path = os.path.join(output_dir, file_name)

    run_propagation_and_enrichment(file_name, prior_data, network, network_name, alpha, prop_method, output_file_path)
