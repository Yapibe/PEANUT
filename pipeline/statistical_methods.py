...
def wilcoxon_rank_sums_test(
    experiment_scores: list, control_scores: list, alternative: str = "two-sided"
) -> float:
    """
    ...
    p_values = ranksums(experiment_scores, control_scores, alternative=alternative).pvalue
    return p_values
...
def bh_correction(p_values: np.ndarray) -> np.ndarray:
    ...
    Parameters:
    - p_values (np.ndarray): Array of p-values obtained from multiple statistical tests.

    Returns:
    - np.ndarray: Array of adjusted p-values.
    """
    p_values_rank = rankdata(p_values, method='max') - 1
    p_values_rank_ord = rankdata(p_values, method='ordinal') - 1
    p_values_sorted = np.argsort(p_values_rank_ord)
    p_values *= len(p_values) / (p_values_rank + 1)
    adj_p_values_by_rank = p_values[p_values_sorted]
    p_values_ordered = np.minimum.accumulate(adj_p_values_by_rank[::-1])[::-1]
    adj_p_values = p_values_ordered[np.argsort(p_values_sorted)]

    return adj_p_values


def kolmogorov_smirnov_test(experiment_scores: list, control_scores: list) -> float:
    """
    ...
    cdf_experiment = np.searchsorted(experiment_scores, data_all, side='right') / n1
    cdf_control = np.searchsorted(control_scores, data_all, side='right') / n2
...
def compute_mw_python(experiment_ranks: list, control_ranks: list) -> tuple:
    """
...
def kolmogorov_smirnov_test_with_ranking(pathway_genes: set, global_ranking: pd.Series) -> float:
    """
    ...
    # Calculate the KS statistic
    n1 = len(pathway_ranks)
    n2 = len(background_ranks)
    cdf_pathway = np.searchsorted(pathway_ranks, global_ranking, side="right") / n1
    cdf_background = (
        np.searchsorted(background_ranks, global_ranking, side="right") / n2
    )
