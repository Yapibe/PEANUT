...
def process_single_file(network, pathway_file, network_name, alpha, method, file_name):
    ...
    output_dir = os.path.join(output_base_dir, method, network_name, pathway_file, f"alpha_{alpha}")
...
def generate_summary_df(filtered_df):
    ...
    pivot_df = filtered_df.pivot_table(
        ...
        columns='Method', values=['Rank', 'Significant'], aggfunc='first'
    ).reset_index()

    # Flatten the multi-level column index created by pivot_table
    pivot_df.columns = ['Dataset', 'Pathway', 'Density', 'Num Genes', 'Avg Diameter'] + [
    ...
    for method in prop_methods:
        ...
        pivot_df[f'{method} Significant'] = pivot_df.get(f'{method} Significant', np.nan)

    # Calculate the average for all columns (both rank and significant)
    avg_row = pivot_df.mean(numeric_only=True).to_frame().T
    avg_row[['Dataset', 'Pathway', 'Density', 'Num Genes', 'Avg Diameter']] = ['Average', '', '', '', '']
                    ...
                    if pathway_name in loaded_pathways[pathway_file]:
                        for prop_method in prop_methods:
                            futures.append(executor.submit(
                                process_single_file, loaded_networks[network_name], pathway_file, network_name, alpha, prop_method, file_name
                            ))
