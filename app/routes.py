...
async def execute_pipeline(
    ...
    logger.info(
        f"File: {file.filename}, Test Name: {test_name}, Species: {species}, Alpha: {alpha}, Network: {network}, Pathway File: {pathway_file}, Min Genes per Pathway: {min_genes_per_pathway}, Max Genes per Pathway: {max_genes_per_pathway}, FDR Threshold: {fdr_threshold}"
    )
    ...
    try:
        ...
        job_storage[job_code] = {
            ...
            "output_file": None,
            "expiry": datetime.now()
            + timedelta(days=7),  # Set expiry to 7 days from now
        }
        ...
        background_tasks.add_task(
            run_pipeline, file_content, file.filename, settings, job_storage, job_code
        )

        return PipelineOutput(result="Job submitted successfully", job_code=job_code)
